...
from model.spendings import Spend, Currency, str_total
from page.main_page import Period
from page.marks import TestData

TEST_CATEGORY = "образование"

spend_today = Spend.random(category=TEST_CATEGORY,
                           currency=Currency.RUB
                           )
spend_yesterday = Spend.random(category=TEST_CATEGORY,
                               currency=Currency.RUB,
                               days_delta=1
                               )
spend_past_week = Spend.random(category=TEST_CATEGORY,
                               currency=Currency.RUB,
                               days_delta=6
                               )
spend_past_month = Spend.random(category=TEST_CATEGORY,
                                currency=Currency.RUB,
                                days_delta=25
                                )
old_spend = Spend.random(category=TEST_CATEGORY,
                         currency=Currency.RUB,
                         days_delta=60,
                         min_days_delta=32
                         )
